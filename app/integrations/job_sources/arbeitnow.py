...
class ArbeitnowSourceError(Exception):
...
class ArbeitnowFetchResult:
    records: tuple[JobIngestionRecord, ...]
    page: int
    has_next_page: bool
    rejected_count: int
    skipped_non_remote_count: int
...
def _normalize_employment_type(
    ...
    value = _classification_text("", job_types)
...
def _normalize_experience_level(
    ...
    value = _classification_text(title, job_types)
